...
<div class='container bg-white mb-4 py-4'>
    <?php if (!empty($productDocs)) { ?>
        ...
        if ($rest > 0) {
            echo "<div id='loadMore' class='load-more mt-4' data-page='$page' data-size='$pageSize'>";
            echo "Xem thêm <span class='rest'>$rest</span> sản phẩm <i class='bi bi-caret-down-fill'></i><span class='spinner-border d-none'></span>";
            echo "</div>";
        }
        ?>
    <?php } else {
        echo "<p class='text-center text-gray py-4' style='font-size: 1.6rem'>Không tìm thấy sản phẩm nào</p>";
    } ?>
</div>